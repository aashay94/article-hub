...
// Gets a list of Article by particular Category
export function articleCategory(req, res) {
    var category = req.query.category;
    category = category.charAt(0).toUpperCase() + category.slice(1);
    
    return Article.find({category:category}).populate("author").exec()
        .then(respondWithResult(res))
        .catch(handleError(res));
}
...
export function show(req, res) {
    return Article.findById(req.params.id).populate("author").exec()
